...
xhr = xh - [0; pi; 0; 0];
%gain matrices
KK = [10.0000  -91.7720   16.2819  -22.6933];
LL = [0 0];
j=0;
...
subplot(2,2,2);
axis([0 2 -100 100])
plot([(j-1)/100 j/100],[u_old u],'Color','k','LineWidth',2)
xlabel('Time(s)') 
ylabel('u(t)') 
hold on
...
subplot(2,2,3);
axis([0 2 -1 1])
plot([j/100 (j+1)/100],[xh(1,i) xh(1,i+1)],'Color','k','LineWidth',2)
xlabel('Time(s)') 
ylabel('x_1(t)') 
hold on
...
subplot(2,2,4);
axis([0 2 -1 1])
plot([j/100 (j+1)/100],[xh(2,i)-pi xh(2,i+1)-pi],'Color','k','LineWidth',2)
xlabel('Time(s)') 
ylabel('x_2(t)') 
hold on
...
%mapping to real time
mov2 = mov(1:1:200);

%write the video
writerObj = VideoWriter('cartpole_walls_lqr', 'MPEG-4');
